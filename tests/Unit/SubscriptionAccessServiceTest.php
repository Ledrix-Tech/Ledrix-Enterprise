...
class SubscriptionAccessServiceTest extends TestCase
{
    ...
    public function test_past_due_within_grace_keeps_crm_access(): void
    {
        config(['subscription.past_due_grace_days' => 7]);
        $membership = $this->membershipStub('past_due', now()->subDays(3));
        $tenant = $this->tenantStub($membership, onTrial: false);

        $this->assertTrue($this->service->needsPayment($tenant));
        $this->assertTrue($this->service->canUseCrm($tenant));
    }

    public function test_past_due_after_grace_locks_crm(): void
    {
        config(['subscription.past_due_grace_days' => 7]);
        $membership = $this->membershipStub('past_due', now()->subDays(10));
        $tenant = $this->tenantStub($membership, onTrial: false);
