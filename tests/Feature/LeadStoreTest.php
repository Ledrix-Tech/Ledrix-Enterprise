...
class LeadStoreTest extends TestCase
{
    ...
    public function test_leads_route_to_correct_tenant_when_brands_share_host(): void
    {
        Notification::fake();

        $brandA = Brand::factory()->create([
            'tenant_id'  => 1,
            'brand_name' => 'Craftic Designs',
            'brand_url'  => 'https://ledrix.co/projects/craftic-designs/contact',
            'brand_host' => 'ledrix.co',
        ]);
        $brandB = Brand::factory()->create([
            'tenant_id'  => 2,
            'brand_name' => 'Craftic Designs All',
            'brand_url'  => 'https://ledrix.co/projects/craftic-designs-all/contact',
            'brand_host' => 'ledrix.co',
        ]);

        $sellerB = Seller::factory()->create(['brand_id' => $brandB->id, 'tenant_id' => 2]);

        App::instance(\App\Services\LeadAssigner::class, Mockery::mock(\App\Services\LeadAssigner::class, function ($mock) use ($sellerB) {
            $mock->shouldReceive('assignNext')->andReturn($sellerB);
        }));
        ...
        $response = $this->postJson(route('crm.leads.post'), [
            'name'      => 'Shared Host Lead',
            'email'     => 'shared-host@example.com',
            'phone'     => '5550001111',
            'url'       => 'https://ledrix.co/projects/craftic-designs-all/contact',
            'brand_key' => $brandB->public_form_token,
        ], [
            'Origin' => 'https://ledrix.co',
        ]);
    ...
    public function test_shared_host_brands_get_unique_script_urls(): void
    {
        $brandA = Brand::factory()->create([
            'tenant_id'  => 1,
            'brand_url'  => 'https://ledrix.co/projects/craftic-designs',
            'brand_host' => 'ledrix.co',
        ]);
        $brandB = Brand::factory()->create([
            'tenant_id'  => 2,
            'brand_url'  => 'https://ledrix.co/projects/craftic-designs-all',
            'brand_host' => 'ledrix.co',
        ]);

        $service = app(LeadScriptService::class);

        $this->assertSame(route('lead.script', ['host' => 'brand-' . $brandA->id]), $service->scriptUrlForBrand($brandA));
        $this->assertSame(route('lead.script', ['host' => 'brand-' . $brandB->id]), $service->scriptUrlForBrand($brandB));
        $this->assertNotSame($service->scriptUrlForBrand($brandA), $service->scriptUrlForBrand($brandB));
    }
}
