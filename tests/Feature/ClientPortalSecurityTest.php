...
class ClientPortalSecurityTest extends TestCase
{
    ...
    public function test_client_portal_stays_open_when_tenant_saas_subscription_expired(): void
    {
        // Agency CRM is billing-gated; end-clients must still see project data.
        $this->mock(SubscriptionAccessService::class, function ($mock) {
            $mock->shouldReceive('canUseCrm')->andReturn(false);
        });

        $client = $this->createPortalClient([
            'email' => 'client-open@example.com',
        ]);

        $this->actingAs($client, 'client')
            ->get(route('client.index.get'))
            ->assertOk();
    }
