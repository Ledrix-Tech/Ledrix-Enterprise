...
class TwoFactorController extends Controller
{
    ...
    public function disable(Request $request, TotpService $totp)
    {
        ...
        if (config('security.force_super_admin_owner_2fa') && $admin->isOwner()) {
            return back()->with('error', 'Platform owner two-factor authentication cannot be disabled while it is required.');
        }
