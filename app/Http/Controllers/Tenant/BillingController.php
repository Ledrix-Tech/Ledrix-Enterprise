...
class BillingController extends Controller
{
    ...
    public function index(
        ...
        $displayAmount = $pricing['amounts'][$billingCurrency]
            ?? ($isPakistanBuyer ? $pricing['pkr'] : ($pricing['usd'] ?? 0));
        $billingRegionLabel = TenantBillingRegion::regionLabel($tenant);
        $supportedCurrencies = TenantBillingRegion::supportedCurrencies();
        $stripePortalReady = $stripeReady && filled($tenant->stripe_customer_id);
    ...
    public function updateAutoRenew(Request $request, SubscriptionAccessService $accessService, TenantStripeCheckoutService $stripeCheckout)
    {
        $tenant = $this->organizationTenant();
        ...
        if ($membership?->stripe_subscription_id) {
            try {
                $stripeCheckout->cancelAtPeriodEnd($membership, cancel: ! (bool) $validated['auto_renew']);
            } catch (\Throwable $e) {
                report($e);
            }
        }
    ...
    public function cancelAtPeriodEnd(Request $request, SubscriptionAccessService $accessService, TenantStripeCheckoutService $stripeCheckout)
    {
        $tenant = $this->organizationTenant();
        $membership = $accessService->currentMembership($tenant);
        ...
        if ($membership->stripe_subscription_id) {
            try {
                $stripeCheckout->cancelAtPeriodEnd($membership, cancel: true);
            } catch (\Throwable $e) {
                report($e);
            }
        }
    ...
    public function updateBillingCurrency(Request $request)
    {
        ...
        $validated = $request->validate([
            'preferred_billing_currency' => ['required', 'string', Rule::in(TenantBillingRegion::supportedCurrencies())],
        ]);

        $currency = strtoupper($validated['preferred_billing_currency']);
        $tenant->forceFill([
            'preferred_billing_currency' => $currency,
        ])->save();

        $label = TenantBillingRegion::regionLabel($tenant->fresh());
