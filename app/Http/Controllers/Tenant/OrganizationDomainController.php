...
class OrganizationDomainController extends Controller
{
    ...
    public function edit(CustomDomainVerificationService $domains, TenantFeatureService $features)
    {
        ...
        return $this->organizationView('domain', [
            ...
            'workspaceBaseUrl'  => \App\Support\TenantHostResolver::workspaceBaseUrlForTenant($tenant),
            'workspacePanelUrls'=> \App\Support\TenantHostResolver::workspacePanelUrlsForTenant($tenant),
        ]);
    }
    ...
    public function updateDomain(Request $request, CustomDomainVerificationService $domains, TenantFeatureService $features)
    {
        ...
        if (\App\Support\TenantHostResolver::isPlatformWorkspaceHost($domain)) {
            throw ValidationException::withMessages([
                'custom_domain' => 'Use your default workspace URL ('.\App\Support\TenantHostResolver::workspaceHostForSlug($tenant->slug).') — no setup needed. This field is only for domains you own outside ledrix.co.',
            ]);
        }
