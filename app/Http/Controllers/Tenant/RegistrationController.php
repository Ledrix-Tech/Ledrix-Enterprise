...
class RegistrationController extends Controller
{
    ...
    public function store(
        ...
        $payload = [
            ...
            'trial_days' => (int) ($result['tenant']->plan?->trial_days ?? 0),
        ];
    ...
    public function success()
    {
        ...
        return view('front.pages.tenant.register-success', [
            'email'     => session('email'),
            'plan'      => session('plan'),
            'trialDays' => (int) session('trial_days', 0),
    ...
    public function verify(string $token, VerifyTenantEmailService $verifyTenantEmailService)
    {
        ...
        if ($tenant) {
            ...
            return redirect()
                ...
                ->with('success', $tenant->isOnTrial()
                    ? 'Email verified! Your free trial is now active.'
                    : 'Email verified! Complete billing to activate your workspace.');
        }
