...
class StripeBillingController extends Controller
{
    ...
    public function checkout(
        ...
        if (TenantBillingRegion::isPakistanBuyer($tenant)) {
            return $this->organizationRedirect(
                ...
                [],
                'error',
                'Stripe is for international billing. Switch region or pay with Meezan (PKR).'
            );
        }
        ...
        try {
            ...
            $result = $invoiceService->createForTenant($tenant, 'stripe', $currency, $orderType);
    ...
    /** F-26: Redirect to Stripe Customer Portal. */
    public function portal(TenantStripeCheckoutService $stripeCheckout)
    {
        $tenant = $this->organizationTenant();
        ...
        if (TenantBillingRegion::isPakistanBuyer($tenant)) {
            return $this->organizationRedirect(
                ...
                [],
                'error',
                'Stripe Customer Portal is for international (Stripe) billing only.'
            );
        }

        try {
            $url = $stripeCheckout->createBillingPortalUrl(
                $tenant,
                route($this->organizationRouteName('billing'))
            );
        } catch (RuntimeException $e) {
            return $this->organizationRedirect('billing', [], 'error', $e->getMessage());
        }
        ...
        try {
            $recorder->recordAndProcess(
                ...
                function () use ($activationService, $payment, $sessionId) {
                    ...
                    $stripeCheckout->syncSubscriptionIdsFromSession(
                        ($fresh ?? $payment)->fresh(['tenant', 'membership']),
                        $sessionId
                    );
                },
            );
