...
class AdminAuthController extends Controller
{
    ...
    protected function resolveAdminForLogin(string $email, string $password): ?Admin
    {
        ...
        $tenantId = TenantHostResolver::resolveTenantId(request());

        if ($tenantId !== null) {
            $matches = $matches->where('tenant_id', $tenantId)->values();
        }
    ...
    public function adminForgotPost(Request $request)
    {
        ...
        // Forgot/reset runs without an authenticated tenant context — scope by host when on custom domain.
        $adminQuery = Admin::withoutGlobalScopes()->where('email', $request->email);
    ...
    public function adminResetPost(Request $request)
    {
        ...
        // Update password in the right table (unscoped — guest reset; filter by host tenant when present).
        $adminQuery = Admin::withoutGlobalScopes()->where('email', $request->email);
        $sellerQuery = Seller::withoutGlobalScopes()->where('email', $request->email);
