...
class AdminTwoFactorController extends Controller
{
    ...
    public function disable(Request $request, TotpService $totp)
    {
        ...
        if (config('security.force_tenant_admin_2fa') && ($admin->role ?? null) === 'admin') {
            return back()->with('error', 'Organization owner two-factor authentication cannot be disabled while it is required.');
        }
