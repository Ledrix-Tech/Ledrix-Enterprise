...
class LeadIntakeService
{
    ...
    public function resolveBrand(?string $url, Request $req): ?Brand
    {
        ...
        return $this->resolveBrandFromHostUrl($url ?: $referer)
    ...
    public function resolveBrandFromHostUrl(?string $url): ?Brand
    {
        ...
        return Brand::resolveByHostKey(self::normalizeHost($url) ?? '', $url);
    }

    public function resolveBrandFromRequestOrigin(Request $req): ?Brand
    {
        $referer = $req->headers->get('Referer');
        $origin = $req->headers->get('Origin') ?: $referer;

        if (! $origin) {
            return null;
        }

        return Brand::resolveByHostKey(self::normalizeHost($origin) ?? '', $referer ?: $origin);
    }
