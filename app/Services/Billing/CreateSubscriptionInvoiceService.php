...
class CreateSubscriptionInvoiceService
{
    ...
    public function createForTenant(
        ...
        ?PackagePricing $planOverride = null,
        ...
        $plan = $planOverride ?? $tenant->plan ?? $membership->plan;

        if (! $plan) {
            throw new RuntimeException('No plan found for this tenant.');
        }
        ...
        // Keep $plan from override / tenant resolution above (do not reset to tenant.plan only).
        $baseAmount = $amountOverride !== null
            ? max(0, (float) $amountOverride)
            : $this->pricingService->resolveAmount($plan, $billingCycle, $currency);

        if ($baseAmount <= 0 && $amountOverride === null) {
        ...
        return DB::connection('central')->transaction(function () use (
            ...
            $subtotal = (float) $adjustment['amount'];
            $taxRate = max(0.0, (float) config('services.invoice_tax.rate', 0));
            $taxAmount = $subtotal > 0 && $taxRate > 0
                ? BillingMoney::round($subtotal * ($taxRate / 100), $currency)
                : 0.0;
            $amount = BillingMoney::round($subtotal + $taxAmount, $currency);
            ...
            if ($taxAmount > 0) {
                ...
                $invoiceNotes .= ' · ' . $taxLabel . ' ' . rtrim(rtrim(number_format($taxRate, 2, '.', ''), '0'), '.') . '%';
            }
    ...
    public function createAndNotify(
        ...
        $result = $this->createForTenant($tenant, $gateway, $currency, $orderType, $planOverride, $amountOverride);
