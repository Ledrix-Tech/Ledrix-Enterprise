...
class ActivateTenantSubscriptionService
{
    ...
    public function activate(
        ...
        $activated = DB::connection('central')->transaction(function () use (
            $payment,
            $renewedBy,
            $note,
            $payloadMerge,
            ...
            $pendingPlanId = (int) (($tenantMeta['pending_plan_change']['plan_id'] ?? 0));
            if ($pendingPlanId > 0 && (int) $plan->id === $pendingPlanId) {
                unset($tenantMeta['pending_plan_change']);
            } elseif ($pendingPlanId > 0 && in_array($payment->order_type, ['renewal', 'new'], true)) {
                $pendingPlan = PackagePricing::query()->find($pendingPlanId);
                if ($pendingPlan) {
                    $plan = $pendingPlan;
                }
                unset($tenantMeta['pending_plan_change']);
        ...
        app(\App\Services\Tenant\TenantWebhookDispatcher::class)
            ->dispatchSubscriptionActivated($activated);
