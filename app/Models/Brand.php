...
class Brand extends Model
{
    ...
    public static function resolveFromUrl(?string $url): ?self
    {
        ...
        return self::resolveByHostKey(self::normalizeHost($url) ?? '', $url);
    }

    /**
     * Resolve a brand from a script route key (hostname or brand-{id}) and optional full page URL.
     */
    public static function resolveByHostKey(string $hostKey, ?string $fullUrl = null): ?self
    {
        $hostKey = strtolower(trim(str_replace('.js', '', $hostKey)));

        if ($hostKey === '') {
            return null;
        }

        if (preg_match('/^brand-(\d+)$/', $hostKey, $matches)) {
            return self::withoutGlobalScopes()->find((int) $matches[1]);
        }

        $host = self::normalizeHost($hostKey);
        if (! $host) {
            return null;
        }

        $candidates = self::withoutGlobalScopes()
        ...
        if ($fullUrl) {
            return self::matchByUrlPath($candidates, $fullUrl);
        }

        return null;
    }

    /** @param  iterable<int, self>  $candidates */
    public static function matchByUrlPath(iterable $candidates, string $fullUrl): ?self
    {
        $incomingPath = self::normalizeUrlPath($fullUrl);
        if (! $incomingPath) {
            return null;
        }
        ...
        foreach ($candidates as $brand) {
            $prefix = self::normalizeUrlPath($brand->brand_url);
            if (! $prefix) {
                continue;
            }

            if (str_starts_with($incomingPath, $prefix) && strlen($prefix) > $bestLength) {
                $best = $brand;
    ...
    public static function normalizeUrlPath(?string $url): ?string
    {
        if (! $url) {
            return null;
        }
    ...
    public static function hostIsShared(string $host): bool
    {
        $host = self::normalizeHost($host);

        return (bool) $host
            && self::withoutGlobalScopes()->where('brand_host', $host)->count() > 1;
    }

    /** Unique key for /lead-script/{key}.js — avoids one script serving multiple brands on the same host. */
    public function scriptRouteKey(): string
    {
        $host = $this->brand_host;

        if ($host && ! self::hostIsShared($host)) {
            return $host;
        }
