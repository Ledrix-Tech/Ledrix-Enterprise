...
use App\Http\Controllers\API\Client\ClientTwoFactorController;
...
Route::group(['prefix' => 'client', 'namespace' => 'Client'], function () {
    ...
    Route::get('/2fa/challenge', [ClientTwoFactorController::class, 'challenge'])->name('client.2fa.challenge');
    Route::post('/2fa/challenge', [ClientTwoFactorController::class, 'verifyChallenge'])->name('client.2fa.challenge.post');
    ...
    Route::group(['middleware' => 'client'], function () {
        Route::get('/2fa', [ClientTwoFactorController::class, 'showSetup'])->name('client.2fa.setup');
        Route::post('/2fa/enable', [ClientTwoFactorController::class, 'enable'])->name('client.2fa.enable');
        Route::post('/2fa/disable', [ClientTwoFactorController::class, 'disable'])->name('client.2fa.disable');

        Route::get('/dashboard', [ClientPagesController::class, 'clientIndex'])->name('client.index.get');
        Route::get('/messages', [\App\Http\Controllers\API\Client\MessageController::class, 'index'])->name('client.messages.get');
        Route::post('/messages/{order}', [\App\Http\Controllers\API\Client\MessageController::class, 'store'])
            ->name('client.messages.store')
            ->whereNumber('order');
