...
Route::group(['prefix' => 'admin'], function () {
    ...
    Route::get('/sso/redirect', [AdminSsoController::class, 'redirect'])->name('admin.sso.redirect');
    Route::get('/sso/callback', [AdminSsoController::class, 'callback'])->name('admin.sso.callback');

    // Exit impersonation without super_admin middleware (session must have impersonator).
    ...
    Route::group(['middleware' => ['admin', 'finance.restrict', '2fa.forced:admin']], function () {
    ...
    Route::group(['middleware' => ['admin', 'finance.restrict', '2fa.forced:admin', 'crm.workspace', 'tenant.maintenance', 'tenant.feature:ppc_module']], function () {
        ...
        Route::middleware('admin.only')->group(function () {
            ...
            Route::get('/export/{table?}', [ExportController::class, 'exportLeadsCsv'])->name('export.csv')->whereIn('table', ['leads', 'clients', 'orders', 'payments', 'client_tickets']);
    ...
    Route::group(['middleware' => ['admin', 'finance.restrict', '2fa.forced:admin', 'crm.workspace', 'tenant.maintenance', 'admin.only']], function () {
        Route::prefix('organization')->name('admin.org.')->group(function () {
            ...
            Route::post('/plan/change', [OrganizationPlanController::class, 'change'])->name('plan.change');
            ...
            Route::middleware('tenant.feature:api_access')->group(function () {
                ...
                Route::get('/webhooks', [\App\Http\Controllers\Tenant\OrganizationWebhookEndpointController::class, 'index'])->name('webhooks');
                Route::post('/webhooks', [\App\Http\Controllers\Tenant\OrganizationWebhookEndpointController::class, 'store'])->name('webhooks.store');
                Route::post('/webhooks/{id}/toggle', [\App\Http\Controllers\Tenant\OrganizationWebhookEndpointController::class, 'toggle'])
                    ->name('webhooks.toggle')
                    ->whereNumber('id');
                Route::delete('/webhooks/{id}', [\App\Http\Controllers\Tenant\OrganizationWebhookEndpointController::class, 'destroy'])
            ...
            Route::get('/billing/invoices/{invoice}/pdf', [TenantInvoiceController::class, 'downloadPdf'])->name('billing.invoice.pdf')->whereNumber('invoice');
            ...
            Route::post('/billing/stripe/portal', [StripeBillingController::class, 'portal'])->name('billing.stripe.portal');
