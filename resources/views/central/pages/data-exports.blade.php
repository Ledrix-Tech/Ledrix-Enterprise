                            ...
                            <tr>
                                ...
                                <td data-label="Action">
                                    @if ($export->isReady())
                                        ...
                                        @if ($canManage)
                                            <form method="POST" action="{{ route('super-admin.data-exports.restore-dry-run', $export->id) }}" class="d-inline">
                                                @csrf
                                                <button class="btn btn-sm btn-outline-secondary" type="submit">Restore (dry-run)</button>
                                            </form>
                                            <form method="POST" action="{{ route('super-admin.data-exports.restore', $export->id) }}" class="d-inline mt-1"
                                                onsubmit="return confirm('WRITE CRM rows from this export into the live primary DB for tenant #{{ $export->tenant_id }}? This cannot be undone easily.');">
                                                @csrf
                                                <button class="btn btn-sm btn-outline-danger" type="submit">Restore (force)</button>
                                            </form>
                                        @endif
                                    @endif
