...
@section('main-content')
    <header class="hero d-flex align-items-center justify-content-center text-center"
        ...
        <div class="container text-white">
            <h1>{{ $tenant->name }}</h1>
            <p class="mb-0">{{ \App\Support\TenantHostResolver::workspaceHostForSlug($tenant->slug) }} · {{ $tenant->email }}</p>
        </div>
    </header>
    ...
    <main class="py-5">
        <div class="container">
            ...
            @php
                $workspaceHost = \App\Support\TenantHostResolver::workspaceHostForSlug($tenant->slug);
                $workspacePanels = \App\Support\TenantHostResolver::workspacePanelUrlsForTenant($tenant);
            @endphp
            <div class="alert alert-light border mb-4">
                <strong>Your workspace URL:</strong>
                <code>{{ $workspaceHost }}</code>
                — <a href="{{ $workspacePanels['admin'] }}" target="_blank" rel="noopener">Open Admin CRM</a>
                · <a href="{{ route('tenant.domain') }}">Domain &amp; branding</a>
                <div class="small text-muted mt-1">Ready to use — no DNS setup required.</div>
            </div>
